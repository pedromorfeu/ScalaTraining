...
object Huffman {
  ...
  def decode(tree: CodeTree, partialTree: CodeTree, bits: List[Bit]): List[Char] =
    partialTree match {
      case Leaf(char, _) => {
        ...
        else List(char) ::: decode(tree, tree, bits)
      }
  ...
  def getChar(tree: CodeTree, bits: List[Bit]): Char = tree match {
    case Leaf(char, _) => char
    case Fork(left, right, chars, weight) => if (bits.head == 0) getChar(left, bits.tail) else getChar(right, bits.tail)
  }

  def getBits(tree: CodeTree, char: Char): List[Bit] = getBits(tree, char, List())
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    else getBits(tree, text.head, List()) ::: encode(tree)(text.tail)
  ...
  /**
    ...
    */
  def convert(tree: CodeTree): CodeTable = convert(tree, tree)

  def convert(tree: CodeTree, partialTree: CodeTree): CodeTable = partialTree match {
    case Leaf(char, _) => List((char, getBits(tree, char)))
    case Fork(left, right, _, _) => convert(tree, left) ::: convert(tree, right)
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    ...
    else if(!contains(a, b.head._1)) List(b.head) ::: mergeCodeTables(a, b.tail)
    else mergeCodeTables(a, b.tail)

  def contains(table: CodeTable, char: Char): Boolean =
    if(table.isEmpty) false
    else if (char == table.head._1) true
    else contains(table.tail, char)
